...
export class BuildsClient {
    ...
    getEngineBuilds(): Promise<EngineForBuild[]> {
        let url_ = this.baseUrl + "/api/Builds/engine-builds";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetEngineBuilds(_response);
        });
    }

    protected processGetEngineBuilds(response: Response): Promise<EngineForBuild[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
                let result200: any = null;
                result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as EngineForBuild[];
                return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<EngineForBuild[]>(null as any);
    }

    getDefaultEngineForBuild(project: string | undefined, sha: string | undefined): Promise<string> {
        let url_ = this.baseUrl + "/api/Builds/default-engine-build?";
        if (project === null)
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetDefaultEngineForBuild(_response);
        });
    }

    protected processGetDefaultEngineForBuild(response: Response): Promise<string> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export interface EngineForBuild {
    name: string;
    path: string;
}
