...
import {
    ComboboxContentProps, ComboboxErrorMessageProps,
    ComboboxInputProps,
    ComboboxItemProps, ComboboxLabelProps,
    ComboboxTriggerProps,
} from "@kobalte/core/combobox";
import { Combobox as ComboboxPrimitive } from "@kobalte/core/combobox";
import type { PolymorphicProps } from "@kobalte/core/polymorphic";
import {ParentProps, Show, ValidComponent, VoidProps} from "solid-js";
import { splitProps } from "solid-js";
import {cva} from "class-variance-authority";
import {TextField as TextFieldPrimitive, TextFieldLabelProps} from "@kobalte/core/text-field";
...
export const ComboboxInput = <T extends ValidComponent = "input">(
    ...
    return (
        <ComboboxPrimitive.Input
            class={cn(
                "h-7 bg-transparent text-sm placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 grow",
                local.class,
            )}
            {...rest}
        />
...
export const comboBoxLabel = cva(
...
export const ComboboxLabel = <T extends ValidComponent = "label">(
    ...
    return (
        <ComboboxPrimitive.Label
            class={cn(comboBoxLabel(), local.class)}
            {...rest}
        />
    );
};
...
export const ComboboxErrorMessage = <T extends ValidComponent = "div">(
    ...
    return (
        <ComboboxPrimitive.ErrorMessage
            class={cn(comboBoxLabel({ error: true }), local.class)}
            {...rest}
        />
    );
};
    ...
    return (
        <ComboboxPrimitive.Control>
            <ComboboxPrimitive.Trigger
                class={cn(
                    "flex h-fit w-full py-2 items-center justify-between rounded-md border border-input px-3 shadow-sm",
                    local.class,
                )}
                {...rest}
            >
                {local.children}
                <ComboboxPrimitive.Icon class="flex h-6 w-3.5 items-center justify-center">
                    <svg
...
export const ComboboxContent = <T extends ValidComponent = "div">(
    ...
    const [local, rest] = splitProps(props as comboboxContentProps, ["class", "listClass"]);
    ...
    return (
        <ComboboxPrimitive.Portal>
            <ComboboxPrimitive.Content
                ...
                {...rest}
            >
                <ComboboxPrimitive.Listbox class={cn("p-1", local.listClass)} />
            </ComboboxPrimitive.Content>
        </ComboboxPrimitive.Portal>
    );
};
...
export const ComboboxItem = <T extends ValidComponent = "li">(
    ...
    return (
        <ComboboxPrimitive.Item
            class={cn(
                "relative flex grow-1 w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none data-[disabled]:pointer-events-none data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground data-[disabled]:opacity-50",
                local.class,
            )}
            {...rest}
        >
            ...
            <ComboboxPrimitive.ItemLabel class="w-full">
                {local.children}
